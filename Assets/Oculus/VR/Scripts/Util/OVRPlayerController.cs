...
public class OVRPlayerController : MonoBehaviour
{
	...
	private bool SkipMouseRotation; // It is rare to want to use mouse movement in VR, so ignore the mouse by default.
	...
	void Start()
	{
		...
		if (SceneManager.GetActiveScene().name == "Hospital") SkipMouseRotation = true;
		else SkipMouseRotation = false;
	}
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableRotation)
		{
			...
			{
					euler.y += Input.GetAxis("Mouse X") * rotateInfluence * 2.25f;

					//Borrar esto para version de OCULUS
					euler.x -= Input.GetAxis("Mouse Y") * rotateInfluence * 2.25f;
